...
const extractRawTransaction = (unsigned: TransactionRequest) => {
  return serialize({
...
describe('Native ETH value integration tests', () => {
  ...
  it('should allow an L2 EOA to send to a new account and back again', async () => {
    ...
    let unsigned = await env.l2Wallet.populateTransaction({
      to: other.address,
      value,
      gasPrice: await gasPriceForL2(),
    })
    let raw = extractRawTransaction(unsigned)
    const l1FeeThere =
      await env.messenger.contracts.l2.OVM_GasPriceOracle.connect(
        gasPriceOracleWallet
      ).getL1Fee(raw)
    const there = await wallet.sendTransaction(unsigned)
    ...
    const thereWithGas = initialBalances[0]
      .sub(value)
      .sub(thereGas)
      .sub(l1FeeThere)
    ...
    unsigned = await other.populateTransaction({
      to: wallet.address,
      value: backVal,
      gasPrice: await gasPriceForL2(),
    })
    raw = extractRawTransaction(unsigned)
    const l1FeeBack =
      await env.messenger.contracts.l2.OVM_GasPriceOracle.connect(
        gasPriceOracleWallet
      ).getL1Fee(raw)
    const backAgain = await other.sendTransaction(unsigned)
    ...
    expectBalancesWithinRange(
      ...
      initialBalances[0].sub(thereGas).sub(backVal).sub(l1FeeThere)
    )
    expectBalancesWithinRange(
      backBalances[1],
      initialBalances[1].add(backVal).sub(backGas),
      initialBalances[1].add(backVal).sub(backGas).sub(l1FeeBack)
    )
  })
