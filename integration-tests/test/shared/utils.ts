...
const procEnv = cleanEnv(process.env, {
  ...
  L1ERC721Bridge: addressValidator({
    default: '',
  }),
  L1_FPE: addressValidator({
...
let gasPriceOracleWallet
if (procEnv.GAS_PRICE_ORACLE_KMS_ID !== '') {
  gasPriceOracleWallet = new AwsKmsSigner(
    { keyId: procEnv.GAS_PRICE_ORACLE_KMS_ID, region: procEnv.REGION },
    l2Provider
  )
  console.log(
    'Loaded gas price oracle wallet from KMS id:',
    procEnv.GAS_PRICE_ORACLE_KMS_ID
  )
} else {
  gasPriceOracleWallet = new Wallet(
    procEnv.GAS_PRICE_ORACLE_PRIVATE_KEY,
    l2Provider
  )
  console.log(
    'Loaded gas price oracle wallet from private key, address:',
    gasPriceOracleWallet.address
  )
}
export { gasPriceOracleWallet }
