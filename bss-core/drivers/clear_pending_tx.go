...
func ClearPendingTx(
	...
	updateGasPrice := func(
		...
		signedTx, err := SignClearingTx(
			name, ctx, walletAddr, nonce, l1Client, privKey, kmsClient, kmsID, chainID,
		)
		if err != nil {
...
func SignClearingTx(
	...
	if kmsID != "" {
		transactOpts, err := ethawskmssigner.NewAwsKmsTransactorWithChainID(kmsClient, kmsID, chainID)
		if err != nil {
			return nil, err
		}
