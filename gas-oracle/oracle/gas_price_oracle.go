...
import (
	...
	ethawskmssigner "github.com/ConstellationCrypto/go-ethereum-kms-signer"
...
var (
	...
	// errOneKeyConfig represents the error when exactly one way to configure
	// the gas oracle key is not supplied
	errOneKeyConfig = errors.New("exactly one key configuration must be provided")
...
func (g *GasPriceOracle) Start() error {
	...
	if (g.config.privateKey == nil) == (g.kmsClient == nil) {
		return errOneKeyConfig
	}

	var err error	
	var address common.Address
	if (g.config.kmsID != ""){
		address, err = ethawskmssigner.GetAddress(g.kmsClient, g.config.kmsID)
		if err != nil {
			return err
		}
	} else {
...
func (g *GasPriceOracle) BaseFeeLoop() {
	...
	updateBaseFee, err := wrapUpdateBaseFee(g.l1Backend, g.l2Backend, g.kmsClient, g.config)
	if err != nil {
		panic(err)
	}
...
func NewGasPriceOracle(cfg *Config) (*GasPriceOracle, error) {
	...
	if (cfg.privateKey != nil) == (cfg.kmsID != "") {
		return nil, errOneKeyConfig
	}

	var kmsClient *kms.Client
	if (cfg.kmsID != ""){
		log.Info("Using KMS in region: ", "region:", cfg.region)
	...
	updateL2GasPriceFn, err := wrapUpdateL2GasPriceFn(l2Client, kmsClient, cfg)
	if err != nil {
		return nil, err
	}
...
func ensureConnection(client *ethclient.Client) error {
	...
	for ; true; <-t.C {
		...
		if err == nil {
			...
			retries += 1
			// Set to 5000 to make sure the gas price oracle does not time out during deployment
			if retries > 5000 {
				return err
			}
		}
