...
func NewConfig(ctx *cli.Context) *Config {
	...
	cfg.kmsID = ctx.GlobalString(flags.KMSIDFlag.Name)
	cfg.region = ctx.GlobalString(flags.RegionFlag.Name)
	if ctx.GlobalIsSet(flags.PrivateKeyFlag.Name) && ctx.GlobalString(flags.PrivateKeyFlag.Name) != "" {
	...
	if (cfg.kmsID != "") == (ctx.GlobalString(flags.PrivateKeyFlag.Name) != "") {
		log.Crit("One of KMS ID or Private Key must be set")
	}
