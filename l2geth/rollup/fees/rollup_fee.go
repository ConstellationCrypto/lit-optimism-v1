...
// CalculateTxL1Fee will calculate the total L1 fee given a transaction.
// This function is used at the RPC layer
func CalculateTxL1Fee(tx *types.Transaction, gpo RollupOracle) (*big.Int, error) {
...
func CalculateTotalFee(tx *types.Transaction, gpo RollupOracle) (*big.Int, error) {
	l1Fee, err := CalculateTxL1Fee(tx, gpo)
	if err != nil {
		return nil, err
	}
