...
func (s *SyncService) verifyFee(tx *types.Transaction) error {
	...
	// Check if the network is using FPE as the fee token
	feeTokenSelection := state.GetFeeTokenSelection()
	...
	cost := tx.Value()
	if !isFPEFeeTokenSelect && tx.GasPrice().Cmp(common.Big0) != 0 {
		cost = cost.Add(cost, fee)
	}

	// Ensure that the FPE balance is enough for the gas fee + value
	if isFPEFeeTokenSelect && tx.GasPrice().Cmp(common.Big0) != 0 {
		l1Fee, err := fees.CalculateTxL1Fee(tx, s.RollupGpo)
		if err != nil {
			return fmt.Errorf("invalid transaction: %w", err)
		}
		FPEPriceRatio := state.GetFPEPriceRatio()
		FPEL1Cost := fees.MulByFloat(l1Fee, FPEPriceRatio)
		l2GasLimit := new(big.Int).SetUint64(tx.Gas())
		l2Fee := new(big.Int).Mul(tx.GasPrice(), l2GasLimit)
		FPEGasCost := new(big.Int).Add(FPEL1Cost, l2Fee)
		FPECost := new(big.Int).Add(FPEGasCost, cost)
		if state.GetFPEBalance(from).Cmp(FPECost) < 0 {
			return fmt.Errorf("invalid transaction: %w", core.ErrInsufficientFPEFunds)
		}
	}

	// If FPE, this is fine because state.GetBalance returns the FPE balance
