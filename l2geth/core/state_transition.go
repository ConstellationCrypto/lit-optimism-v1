...
var (
	...
	errInsufficientFPEBalanceForGas = errors.New("insufficient FPE balance to pay for gas")
)
...
type StateTransition struct {
	...
	// UsingOVM
	l1Fee               *big.Int
	isFPEFeeTokenSelect bool
	FPEPriceRatio       *big.Float
}
...
func NewStateTransition(evm *vm.EVM, msg Message, gp *GasPool) *StateTransition {
	...
	// Check if the network has set FPE as the fee token
	// If yes, set gasPrice to 0. This change is what will
	// zero the ETH fee for this transaction.
	feeTokenSelection := evm.StateDB.GetFeeTokenSelection()
	isFPEFeeTokenSelect := feeTokenSelection.Cmp(common.Big1) == 0
	if isFPEFeeTokenSelect {
		gasPrice = big.NewInt(0)
		FPEPriceRatio = evm.StateDB.GetFPEPriceRatio()
	}
	...
	return &StateTransition{
		...
		state:               evm.StateDB,
		l1Fee:               l1Fee,
		isFPEFeeTokenSelect: isFPEFeeTokenSelect,
		FPEPriceRatio:       FPEPriceRatio,
	}
}
...
func (st *StateTransition) buyGas() error {
	...
	if st.state.GetBalance(st.msg.From()).Cmp(mgval) < 0 {
		if rcfg.UsingOVM {
			...
			// in this case. This is a copied chunk of code.
	...
	if st.isFPEFeeTokenSelect {
		ethval = fees.MulByFloat(ethval, st.FPEPriceRatio)
		FPEval = new(big.Int).Add(ethval, new(big.Int).Mul(new(big.Int).SetUint64(st.msg.Gas()), st.msg.GasPrice()))
		if st.state.GetFPEBalance(st.msg.From()).Cmp(FPEval) < 0 {
			return errInsufficientFPEBalanceForGas
		}
	}
	...
	if st.isFPEFeeTokenSelect {
		st.state.SubFPEBalance(st.msg.From(), FPEval)
	} else {
		// else, charge ETH as normal (mgval is not zero)
		// Or, charge zero ETH, since if(isFPEFeeTokenSelect),
...
func (st *StateTransition) TransitionDb() (ret []byte, usedGas uint64, failed bool, err error) {
	...
	st.refundGas()
	if rcfg.UsingOVM {
		// FPE is used to pay for the gas fee
		if st.isFPEFeeTokenSelect {
			ethval := fees.MulByFloat(st.l1Fee, st.FPEPriceRatio)
			FPEval := new(big.Int).Mul(new(big.Int).SetUint64(st.gasUsed()), st.msg.GasPrice())
			FPEval = new(big.Int).Add(ethval, FPEval)
			st.state.AddFPEBalance(dump.OvmFPEGasPriceOracle, FPEval)
		} else {
...
func (st *StateTransition) refundGas() {
	...
	// Return FPE for remaining gas, exchanged at the original rate.
	// Else, return ETH for remaining gas
	if st.isFPEFeeTokenSelect {
		remainingFPE := new(big.Int).Mul(new(big.Int).SetUint64(st.gas), st.msg.GasPrice())
		st.state.AddFPEBalance(st.msg.From(), remainingFPE)
	} else {
