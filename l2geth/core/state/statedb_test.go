...
func TestStateDBFPEAsFeeToken(t *testing.T) {
	...
	FPEBalance := state.GetFPEBalance(addr)
	if FPEBalance.Cmp(common.Big0) != 0 {
		t.Fatal("expect 0 FPE balance")
	}

	// Add FPE balance
	state.AddFPEBalance(addr, common.Big1)
	FPEBalance = state.GetFPEBalance(addr)
	if FPEBalance.Cmp(common.Big1) != 0 {
		t.Fatal("expect 1 FPE balance")
	}

	// Sub FPE balance
	state.SubFPEBalance(addr, common.Big1)
	FPEBalance = state.GetFPEBalance(addr)
	if FPEBalance.Cmp(common.Big0) != 0 {
		t.Fatal("expect 0 FPE balance")
	}
	...
	// Get FPE price ratio
	FPEPriceRatio := state.GetFPEPriceRatio()
	if FPEPriceRatio.Cmp(big.NewFloat(0)) != 0 {
		t.Fatal("expect 0 FPE price ratio")
	}

	// Update FPE price ratio
	state.SetFPEPriceRatio(big.NewInt(1))

	// Get FPE price ratio
	FPEPriceRatio = state.GetFPEPriceRatio()
	if FPEPriceRatio.Cmp(big.NewFloat(1)) != 0 {
		t.Fatal("expect 1 FPE price ratio")
	}

	// Get fee token selection
	isFeeTokenSelected := state.GetFeeTokenSelection()
	if isFeeTokenSelected.Cmp(common.Big0) != 0 {
		t.Fatal("expect 0 fee token selection")
	}

	// Update fee token selection
	state.SetFPEAsFeeToken()
	...
	Balance := state.OVMGetBalance(addr)
	if Balance.Cmp(common.Big0) != 0 {
		t.Fatal("expect 0 balance before adding FPE balance")
	}
	// Add FPE balance
	state.AddFPEBalance(addr, common.Big1)
	FPEBalance = state.GetFPEBalance(addr)
	...
	Balance = state.OVMGetBalance(addr)
	if Balance.Cmp(common.Big1) != 0 {
		t.Fatal("expect 1 balance after adding FPE balance")
	}

	// Add FPE balance
	state.OVMAddBalance(addr, common.Big1)
	FPEBalance = state.GetFPEBalance(addr)
	if FPEBalance.Cmp(common.Big2) != 0 {
		t.Fatal("expect 2 FPE balance")
	}

	state.OVMSetBalance(addr, common.Big3)
	FPEBalance = state.GetFPEBalance(addr)
	if FPEBalance.Cmp(common.Big3) != 0 {
		t.Fatal("expect 3 FPE balance")
	}
}
