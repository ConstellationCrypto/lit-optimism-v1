...
// 1. Waits for a confirmed receipt with more than deployConfig.NUM_DEPLOYMENT_CONFIRMATIONS confirmations.
...
export const getAdvancedContract = (opts: {
  ...
  for (const fnName of Object.keys(contract.functions)) {
    ...
    ;(contract as any)[fnName] = async (...args: any) => {
      ...
      let gasPrice = opts.hre.deployConfig.GAS_PRICE || undefined
      ...
      while (true) {
        ...
        if (receipt === null) {
          ...
          opts.hre.deployConfig.NUM_DEPLOYMENT_CONFIRMATIONS
        ) {
          return tx
        }
      }
...
export const fundAccount = async (
  ...
  if (!hre.deployConfig.IS_FORKED_NETWORK) {
...
export const sendImpersonatedTx = async (opts: {
  ...
  if (!opts.hre.deployConfig.IS_FORKED_NETWORK) {
    throw new Error('this method can only be used against a forked network')
  }
