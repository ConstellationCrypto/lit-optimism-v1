...
import { hexStringEquals, awaitCondition } from '@constellation-labs/core-utils'
...
const deployFn: DeployFunction = async (hre) => {
  ...
  const currentOwner = await Proxy__L1ERC721Bridge.connect(
    Proxy__L1ERC721Bridge.signer.provider
  ).callStatic.getOwner({
  ...
  if (
    ...
    await Proxy__L1ERC721Bridge.connect(owner).setOwner(
      ChugSplashDictator.address,
      {
  ...
  await awaitCondition(
    async () => {
      return hexStringEquals(
        await Proxy__L1ERC721Bridge.connect(
          Proxy__L1ERC721Bridge.signer.provider
        ).callStatic.getOwner({
          from: ethers.constants.AddressZero,
        }),
