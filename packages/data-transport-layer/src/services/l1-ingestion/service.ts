...
import { TypedEvent } from '@constellation-labs/contracts/dist/types/common'
...
export class L1IngestionService extends BaseService<L1IngestionServiceOptions> {
  ...
  private async _syncEvents(
    ...
    // In almost all cases, we won't ever change addresses
    if (this.options.useFixedAddresses) {
      eventRanges.push({
