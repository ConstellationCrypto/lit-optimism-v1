...
import { sleep, remove0x, getChainId } from '@constellation-labs/core-utils'
...
import { queryL1Contracts } from './utils/address-query'
import queryFilter from './utils/query-filter'

/**
 * Initialize the cross - chain messenger with a set of opinionated defaults
 *
 * @param l1Wallet The user's wallet on the L1
 * @param l2Wallet The user's wallet on the L2
 * @param addressEndpoint An endpoint which provides the addresses of the l1 smart contracts
...
export const initializeMessenger = async (
  ...
  opts?: {
    getLogsProvider?: ethers.providers.Provider
    depositConfirmationBlocks?: NumberLike
    l1BlockTimeSeconds?: NumberLike
  }
): Promise<CrossChainMessenger> => {
  const l1AddressDict = await queryL1Contracts(addressEndpoint)
  const bridgeOverrides: BridgeAdapterData = {
    Standard: {
      Adapter: StandardBridgeAdapter,
      l1Bridge: l1AddressDict.L1StandardBridge,
    ...
    NFT: {
      ...
      l1Bridge: l1AddressDict.L1ERC721Bridge,
      l2Bridge: predeploys.L2ERC721Bridge,
    },
  }
  l1SignerOrProvider = toSignerOrProvider(l1SignerOrProvider)
  l2SignerOrProvider = toSignerOrProvider(l2SignerOrProvider)
  const messenger = new CrossChainMessenger({
    l1SignerOrProvider,
    l2SignerOrProvider,
    l1ChainId: await getChainId(
      Provider.isProvider(l1SignerOrProvider)
        ? l1SignerOrProvider
        : l1SignerOrProvider.provider
    ),
    l2ChainId: await getChainId(
    ...
    getLogsProvider: opts?.getLogsProvider,
    depositConfirmationBlocks: opts?.depositConfirmationBlocks,
...
export class CrossChainMessenger implements ICrossChainMessenger {
  ...
  public getLogsProvider: Provider
   ...
   * @param opts.getLogsProvider Provider that can handle eth rpc eth_getLogs requests with unlimited block range
   * @param opts.depositConfirmationBlocks Optional number of blocks before a deposit is confirmed.
   * @param opts.l1BlockTimeSeconds Optional estimated block time in seconds for the L1 chain.
  ...
  constructor(opts: {
    ...
    this.getLogsProvider = opts.getLogsProvider
  ...
  public async toCrossChainMessage(
    ...
    if ((message as CrossChainMessage).message) {
      ...
      (message as BridgeMessage).l1Token &&
      (message as BridgeMessage).l2Token &&
      ...
      // the BridgeMessage (either a ETHDepositInitiated, ERC20DepositInitiated, or
      // WithdrawalInitiated event). We expect the behavior of bridge contracts to be that these
      // BridgeMessage events are triggered and then a SentMessage event is triggered. Our
      ...
      const found = messages
        ...
        .find((m) => {
          // Believe the sentmessage event is the only event triggered in the crosschain messenger
          // prior to the bridge event being emitted, so we've made a change to look for a logIndex
          // for the same tx hash earlier than the bridge event
          return m.logIndex < (message as BridgeMessage).logIndex
        })
  ...
  public async getMessageReceipt(
    ...
    const relayedMessageEvents = await queryFilter(
      messenger,
      messenger.filters.RelayedMessage(messageHash),
      undefined,
      undefined,
      this.getLogsProvider
    )
    ...
    const failedRelayedMessageEvents = await queryFilter(
      messenger,
  ...
  public async getStateBatchAppendedEventByBatchIndex(
    ...
    const events = await queryFilter(
      this.contracts.l1.StateCommitmentChain,
  ...
  public async approveERC721(
    ...
    return (opts?.signer || this.l1Signer).sendTransaction(
      await this.populateTransaction.approveERC721(
        l1Token,
        l2Token,
        TOKEN_ID,
        opts
