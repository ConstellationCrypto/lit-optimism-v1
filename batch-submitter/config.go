...
var (
	// ErrSequencerKeyInput signals that the user tried to set more
	// than one way to pass in the sequencer key
	ErrSequencerKeyInput = errors.New("one of sequencer-private-key " +
		"or mnemonic + sequencer-hd-path or sequencer-kms-id must be set")

	// ErrProposerKeyInput signals that the user tried to set
	// more than one way to pass in the proposer key
	ErrProposerKeyInput = errors.New("one of proposer-private-key " +
		"or mnemonic + proposer-hd-path or proposer-kms-id must be set")

	// ErrSameSequencerAndProposerKMSID signals that the user specified the
	// same KMS id for the proposer and sequencer
	ErrSameSequencerAndProposerKMSID = errors.New("sequencer-kms-id and " +
		"proposer-kms-id must be distinct when using mnemonic")
...
type Config struct {
	...
	// The KMS id of the sequencer key
	SequencerKMSID string

	// The KMS id of the proposer key
	ProposerKMSID string
...
func NewConfig(ctx *cli.Context) (Config, error) {
	cfg := Config{
		...
		SequencerKMSID:		 ctx.GlobalString(flags.SequencerKMSIDFlag.Name),
		ProposerKMSID:		 ctx.GlobalString(flags.ProposerKMSIDFlag.Name),
		Region:		 		 ctx.GlobalString(flags.RegionFlag.Name),
...
func ValidateConfig(cfg *Config) error {
	...
	// Enforce that one of sequencer-private-key or mnemonic + sequencer-hd-path
	// or sequencer-kms-id is enabled
	usingSequencerPrivateKey := cfg.SequencerPrivateKey != ""
	usingSequencerHDPath := cfg.Mnemonic != "" && cfg.SequencerHDPath != ""
	usingSequencerKMSID := cfg.SequencerKMSID != ""
	sequencerKeyInputs := 0
	if usingSequencerPrivateKey {
		sequencerKeyInputs += 1
	}
	if usingSequencerHDPath {
		sequencerKeyInputs += 1
	}
	if usingSequencerKMSID {
		sequencerKeyInputs += 1
	}
	if sequencerKeyInputs != 1 {
		return ErrSequencerKeyInput
	}

	// Enforce that one of either proposer-private-key or mnemonic + proposer-hd-path
	// or proposer-kms-id is enabled

	usingProposerPrivateKey := cfg.ProposerPrivateKey != ""
	usingProposerHDPath := cfg.Mnemonic != "" && cfg.ProposerHDPath != ""
	usingProposerKMSID := cfg.ProposerKMSID != ""
	ProposerKeyInputs := 0
	if usingProposerPrivateKey {
		ProposerKeyInputs += 1
	}
	if usingProposerHDPath {
		ProposerKeyInputs += 1
	}
	if usingProposerKMSID {
		ProposerKeyInputs += 1
	}
	if ProposerKeyInputs != 1 {
		return ErrProposerKeyInput
	}

	// If KMS id is used, the sequencer-kms-id and proposer-kms-id must differ
	if usingSequencerKMSID && usingProposerKMSID &&
		cfg.SequencerKMSID == cfg.ProposerKMSID {
		return ErrSameSequencerAndProposerKMSID
	}
