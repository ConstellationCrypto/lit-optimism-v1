...
func NewDriver(cfg Config) (*Driver, error) {
	...
	if cfg.KMSID != "" {
		walletAddr, err = ethawskmssigner.GetAddress(cfg.KMSClient, cfg.KMSID)
		if err != nil {
			return nil, err
		}
		log.Println(cfg.Name+" wallet params parsed successfully", "wallet_address",
		walletAddr)
	} else {
...
func (d *Driver) ClearPendingTx(
	...
	return drivers.ClearPendingTx(
		...
		d.cfg.KMSClient, d.cfg.KMSID, d.cfg.ChainID,
	)
}
...
func (d *Driver) CraftBatchTx(
	...
	log.Println(name+" crafting batch tx", "start", start, "end", end,
		"nonce", nonce)
	...
	if uint64(len(stateRoots)) < d.cfg.MinStateRootElements {
		log.Println(name+" number of state roots  below minimum",
	...
	log.Println(name+" batch constructed", "num_state_roots", len(stateRoots))

	var opts *bind.TransactOpts
	var err error
	if d.cfg.KMSID != "" {
		opts, err = ethawskmssigner.NewAwsKmsTransactorWithChainID(d.cfg.KMSClient, d.cfg.KMSID, d.cfg.ChainID)
		if err != nil {
			return nil, err
		}
	} else {
	...
	case drivers.IsMaxPriorityFeePerGasNotFoundError(err):
		log.Println(d.cfg.Name + " eth_maxPriorityFeePerGas is unsupported " +
